...
namespace Tests.Util
{
    class IlOp
    {
        ...
        public static IlOp Make(int offset, OpCode opCode)
        {
            return new IlOp(offset, opCode);
        }

        public static IlOp<T> Make<T>(int offset, OpCode opCode, T operand)
        {
            return new IlOp<T>(offset, opCode, operand);
        }
    }

    class IlOp<TArg> : IlOp
    {
        public TArg Operand { get; }

        public IlOp(int offset, OpCode opCode, TArg operand)
            : base(offset, opCode)
        {
    ...
    static class MethodAnalyzer
    {
        ...
        public static IlOp[] ReadIlCode(this MethodBase m)
        {
            var sb = new StringBuilder();
            var c = new IlContext(m);
            var ops = new List<IlOp>();
            ...
            while (!c.EndOfMethodReached)
            {
                ...
                c.Advance(1);
                var oo = _ops.First(op1 => op1.Value == code);

                if (oo.OpCodeType == OpCodeType.Nternal)
                {
                    code = (short)(code << 8 | c.CurrUInt8);
                    oo = _ops.First(op2 => op2.Value == code);
                    c.Advance(1);
                }
                    ...
                    case OperandType.ShortInlineVar:
                        {
                            ...
                            argStr = "$" + c.CurrUInt8;
                            op = IlOp.Make(off, oo, c.Body.LocalVariables[c.CurrUInt8]);
                            c.Advance(1);
                        }
                        break;
                    case OperandType.InlineVar:
                        {
                            ...
                            argStr = "$" + c.CurrInt16;
                            op = IlOp.Make(off, oo, c.Body.LocalVariables[c.CurrInt16]);
                            c.Advance(2);
                        }
                        break;
