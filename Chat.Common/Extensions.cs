...
namespace Chat.Common
{
    ...
    public static class Extensions
    {
        ...
        public static string FormatExceptionOutputInfo(this Exception ex)
        {
            try
            {
                if (ex is IErrorInfoCarrier infoCarrier)
                {
                    return infoCarrier.FormatErrorInfo();
                }
                else
                {
        ...
        private static void FormatExceptionOutputInfoImpl(StringBuilder sb, Exception ex)
        {
            if (ex is IErrorInfoCarrier infoCarrier)
            {
                sb.AppendLine(infoCarrier.FormatErrorInfo());
            }
            else
            {
        ...
        public static void FillWith<T>(this TaskCompletionSource<T> tcs, Func<T> act)
        {
            try
            {
                tcs.SetResult(act());
        ...
        public static bool TryCopyTo(this Stream source, Stream dest, int bytes)
        {
            try
            {
            ...
            {
                System.Diagnostics.Debug.Print("Connection dropped: " + ex.Message);
                return false;
            }
        }
    }
    ...
    public interface IErrorInfoCarrier
    {
        string FormatErrorInfo();
    }
}
